...
def test_posts_index_works(client):
    response = client.get('posts/')
    assert response.status_code == 200

def test_posts_index_try_post_method_should_fail(client):
    response = client.post('posts/')
    assert response.status_code == 405

def test_posts_index_seededContent_exists(client):
    response = client.get('posts/').get_data(as_text=True)
    assert "This is an article" in response
    assert "Minimal Post" in response

def test_posts_create_works(client):
    response = client.get('posts/create')
    assert response.status_code == 200

def test_posts_create_post(client):
...
def test_posts_create_flashMessage_titleEmptyError(client):
...
def test_posts_read_wrongId(client):
